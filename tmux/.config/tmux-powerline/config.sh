...
# General {
	...
	export TMUX_POWERLINE_STATUS_RIGHT_LENGTH="170"

	# Window-row format: the default window list, plus a right-aligned pwd chip.
	# pwd lives here (not in the right status) so the window row carries it.
	# TMUX_POWERLINE_STATUS_FORMAT_WINDOW_DEFAULT comes from config/defaults.sh,
	# which headers.sh sources before this file is read.
	export TMUX_POWERLINE_STATUS_FORMAT_WINDOW="${TMUX_POWERLINE_STATUS_FORMAT_WINDOW_DEFAULT}#[nolist align=right]#[fg=colour166,bg=colour235]#[fg=colour255,bg=colour166,bold] #{=-30:#{s|${HOME}|~|:pane_current_path}} #[default]"
...
# tmux_mem_cpu_load.sh {
	...
	export TMUX_POWERLINE_SEG_TMUX_MEM_CPU_LOAD_ARGS="-v -g 8 -a 1"
# }
